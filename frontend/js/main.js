// Sistema de Gerenciamentode Rotas
    let map;
    let routeMarkers = {};
    let routeLines = {};
    let routes = [];
    ...
    let bairrosDivinopolis = [
      ...
      { id: 2, nome: "São Sebastião", zona: "Norte", checked: false },
      { id: 3, nome: "Santa Rosa", zona: "Sul", checked: false },
      { id: 4, nome: "Esplanada", zona: "Sul", checked: false },
      { id: 5, nome: "Jardim Belvedere", zona: "Leste", checked: false },
      { id: 6, nome: "Niterói", zona: "Leste", checked: false },
      { id: 7, nome: "São Luiz", zona: "Oeste", checked: false },
      { id: 8, nome: "Cidade Jardim", zona: "Oeste", checked: false },
      { id: 9, nome: "Eldorado", zona: "Norte", checked: false },
      { id: 10, nome: "Danilo Passos", zona: "Norte", checked: false },
      { id: 11, nome: "Bom Pastor", zona: "Leste", checked: false },
      { id: 12, nome: "Alto São Paulo", zona: "Leste", checked: false },
      { id: 13, nome: "Interlagos", zona: "Oeste", checked: false },
      { id: 14, nome: "Manoel Valinhas", zona: "Oeste", checked: false },
      { id: 15, nome: "Padre Libério", zona: "Norte", checked: false },
      { id: 16, nome: "Sagrada Família", zona: "Norte", checked: false },
      { id: 17, nome: "Mangabeiras", zona: "Sul", checked: false },
      { id: 18, nome: "Jardinópolis", zona: "Sul", checked: false },
      { id: 19, nome: "Vila Romana", zona: "Leste", checked: false },
      { id: 20, nome: "Catalão", zona: "Oeste", checked: false }
    ];
    ...
    async function chamarApi(acao, opcoes = {}) {
      const token = localStorage.getItem('lixoon_token');
      ...
      if (!resposta.ok) {
        if (resposta.status === 401) {
          localStorage.removeItem('lixoon_token');
          document.getElementById('telaInicial').classList.remove('oculta');
    ...
    async function realizarLogin(event) {
      ...
      try {
        ...
        if (resposta.ok) {
          localStorage.setItem('lixoon_token', data.token);
          fecharModalLogin();
          document.querySelector('.user-name').innerText = data.nome;
          document.querySelector('.user-avatar').innerText = data.nome.substring(0,2).toUpperCase();
          entrarNoSistema();
        ...
        showToast('Erro de conexão ao fazer login', 'error');
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      const token = localStorage.getItem('lixoon_token');
      if (token) {
        // Se já tiver token, tentar carregar dados pra ver se está válido
